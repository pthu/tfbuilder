@node
@author=Clemens Alexandrinus
@brk=*Lib. 3, ed. Mondésert, Matray, and Marrou
@citation_scheme=book//chapter/subchapter/section/line
@cla=Theol.
@convertor_author=Ernst Boogert
@convertor_date=February, 2020
@convertor_execution=Ernst Boogert
@convertor_institution=Protestant Theological University (PThU), Amsterdam/Groningen, The Netherlands
@convertor_version=1.0.0
@date=A.D. 2-3
@description=Paedagogus, ed. H.-I. Marrou, M. Harl, C. Mondésert and C. Matray, Clément d'Alexandrie. Le pédagogue, 3 vols. [Sources chrétiennes 70, 108, 158. Paris: Cerf, 1:1960; 2:1965; 3:1970]: 1:108-294; 2:10-242; 3:12-190. *Lib. 1, ed. Marrou and Harl. *Lib. 2, ed. Mondésert and Marrou. *Lib. 3, ed. Mondésert, Matray, and Marrou. (Cod: 57,864: Phil., Theol.)
@editor=Matray, C.
@epithet=Theol.
@geography=Alexandrinus
@key=0555 002
@pages=1:108-294; 2:10-242; 3:12-190
@place=Paris
@publication_year=1:1960; 2:1965; 3:1970
@publisher=Cerf
@ser=Sources chrétiennes 70, 108, 158
@srt=Clemens 03
@tit=CléMent D'Alexandrie. Le PéDagogue, 3 Vols.
@title=Paedagogus
@title_full=Paedagogus, ed. H.-I. Marrou, M. Harl, C. Mondésert and C. Matray, Clément d'Alexandrie. Le pédagogue, 3 vols. [Sources chrétiennes 70, 108, 158. Paris: Cerf, 1:1960; 2:1965; 3:1970]: 1:108-294; 2:10-242; 3:12-190. *Lib. 1, ed. Marrou and Harl. *Lib. 2, ed. Mondésert and Marrou. *Lib. 3, ed. Mondésert, Matray, and Marrou. (Cod: 57,864: Phil., Theol.)
@tlg_wordcount=57,864
@type=Book in series
@valueType=str
@vid=Scholia: Cf. SCHOLIA IN CLEMENTEM ALEXANDRINUM (5048)
@work=Paedagogus
@xmt=Cod
@writtenBy=Text-Fabric
@dateWritten=2020-02-13T10:48:42Z

5	Paedagogus
